...
本指南面向在 `develop` 分支或本地源码检出中工作的贡献者。完整开发指南（核心命令、设计规则、层边界、工具链）见 [../AGENTS.md](../AGENTS.md)；本页只覆盖分支职责与源码/release CLI 启动方式。

## 分支职责

- `main`：面向正式发布，承载安装脚本、发布说明与用户侧验证入口。
- `develop`：日常开发分支，承载源码迭代与贡献者协作流程。
- `release/vX.Y.Z`：特定版本的发布验证与热修复分支。

开发专用行为应先保留在 `develop` 分支，验证稳定后再提升到 `main`。
