...
- Go TUI Phase 4：slash / tool palette 收口：`clients/go-tui/main.go` 引入 `slashCommand` 类型 + 18 个静态注册表项（/help、/config、/profile(/profiles)、/clear、/exit、/context、/compact、/inbox、/models、/tools、/sessions、/agents、/bash、/read、/grep、/glob、/write、/edit）；`handleLocalCommand` 重写为 registry 查表；live-filter palette（`/` 打开 `modeSlashPick`、实时过滤、Up/Down/Tab 导航、Enter 运行零参或插入 prefix、Esc 关闭）；tool palette（`/tools` 渲染 name/risk/source/approval-required 静态目录，Phase 7 wire `/v1/tools/audit`）；15 个新单测守住 registry 完整性、alias 解析、live filter、backspace、esc、up-down clamp、Enter 零参 / Enter prefix、palette render 隐藏性、tool palette 对齐、`handleLocalCommand` 未知命令错误路径。
