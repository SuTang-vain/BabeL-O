...
| **P2 / Watch** | **SessionChannel TUI 联系可见化** | [active/TODO_tui.md](./active/TODO_tui.md), [reference/session-to-session-memory-channel-plan.md](./reference/session-to-session-memory-channel-plan.md) | `bbl chat` unread/link indicator、`/inbox` side-channel overlay、主对话关键事件卡片与真实 PTY smoke 已落地；支持 message/evidence/governance 展示、ack 与 quote into current prompt，quote 只预填并需手动提交。当前 TUI 作为消费侧入口，发起跨 session message 仍通过 Nexus API 或 AgentScheduler parent-child channel；不得把其他 session 消息渲染为当前用户输入，不做 raw transcript sharing，不允许跨 session 静默改变 cwd/provider/profile/permission。 |
...
5. **P2 / Watch SessionChannel TUI 联系可见化**：LSP context mention、file attachment references、image reference metadata 与 opt-in vim mode 已收口；SessionChannel unread/link indicator、`/inbox` side-channel overlay、主对话关键事件卡片与真实 PTY smoke 已落地；后续只在真实显示 drift 或需要 TUI 发起侧 UX 时重新开项，同时继续守住 side-channel 边界与唯一 input owner。
