...
| **P2** | **SessionChannel TUI 联系可见化** | [active/TODO_tui.md](./active/TODO_tui.md), [reference/session-to-session-memory-channel-plan.md](./reference/session-to-session-memory-channel-plan.md) | Runtime/API 已具备 SessionChannel + Inbox；TUI 仍需把跨 session 联系显示为 side-channel：状态栏 unread/link indicator、Inbox panel/overlay、关键 handoff/blocked/review/memory_candidate 事件卡片、ack/quote 操作与 PTY smoke。不得把其他 session 消息渲染为当前用户输入，不做 raw transcript sharing，不允许跨 session 静默改变 cwd/provider/profile/permission。 |
...
5. **P2 SessionChannel TUI 联系可见化**：LSP context mention、file attachment references、image reference metadata 与 opt-in vim mode 已收口；当前新增打开项是把已落地的 SessionChannel + Inbox 在 TUI 中可见化为状态提示、Inbox overlay 与关键事件卡片，同时守住 side-channel 边界、唯一 input owner 与 PTY smoke。
