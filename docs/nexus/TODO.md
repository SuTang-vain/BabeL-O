...
| **P1** | **Go TUI 任务面板实时更新 + AskUserQuestion 弹窗** | [proposals/go-tui-task-board-and-ask-user-question-plan.md](./proposals/go-tui-task-board-and-ask-user-question-plan.md), [active/TODO_tui.md](./active/TODO_tui.md) | TaskCreate 实时事件 + AskUserQuestion runtime 闭环已落地（Phase 2-6）。Phase 7 验证待收口。 |
| **P1** | **Go TUI Ctrl+D 顶部卡片任务页（翻页）** | [proposals/go-tui-top-card-task-page-plan.md](./proposals/go-tui-top-card-task-page-plan.md), [active/TODO_tui.md](./active/TODO_tui.md) | `/tasks` 斜杠命令在 `m.running` 期间被阻断；Ctrl+D 是唯一可用的运行态监控出口。加 `topCardPage` 分页 + 左右键切换，页面 1 渲染任务列表。Phase 1-4：规划 → 状态+渲染 → 按键处理 → 验证。 |
| **P1** | **Task 工具族补齐：TaskList + TaskUpdate** | [proposals/task-lifecycle-tool-family-expansion-plan.md](./proposals/task-lifecycle-tool-family-expansion-plan.md), [active/TODO_runtime.md](./active/TODO_runtime.md), [active/TODO_tui.md](./active/TODO_tui.md) | LLM 只能通过 `TaskCreate` 创建任务，无法查看列表（`TaskList`）也无法推进状态（`TaskUpdate`）。Phase 0-5：规划文档、TaskList 工具、TaskUpdate 工具、runtime 事件发射、Go TUI 事件处理、验证。 |
...
| **P1 Done** | **Intake 授权连续性与保守性修复** | [proposals/authorization-continuity-execution-plan.md](./proposals/authorization-continuity-execution-plan.md), [proposals/intake-conservatism-analysis.md](./proposals/intake-conservatism-analysis.md), [active/TODO_runtime.md](./active/TODO_runtime.md) | **✅ 已收口（2026-07-10）**：证据 `session_1de7cf54`（Turn 6 "继续任务" 授权重置 → TOOL_DENIED）已修复。Phase 0-4 全部落地：延续指令继承逻辑 + 正则扩展 + Session 级别授权持久化 + intake wiring + Go TUI 状态栏显示。67 个测试通过。设计原则从"默认拒绝，显式授权"调整为"上下文感知，合理推断，关键操作显式确认"。 |
...
| **P2 Plan** | **LLM 网关服务层（OpenAI 兼容入站端点）** | [proposals/llm-gateway-service-plan.md](./proposals/llm-gateway-service-plan.md) | 2026-08-04 spike 已验证（`feat/chat-completions-gateway` 分支）：`POST /v1/chat/completions` 非流式/SSE/`response_format`/模型缺省/错误映射全链路 + `MODEL_NOT_FOUND` 400 守卫修复；首个消费者 AetheL（`babel-ai-engine` M1）。Phase 1：`GET /v1/models` + **非流式快路径**（当前非流式消费者承担流式延迟：直连非流式 175ms vs 经网关 1135ms）+ TTFT +300ms 剖析 + `local` provider 集成测试 + OpenAPI 契约。收口标准：契约稳定、`local` e2e 绿、非流式延迟 ≤ 直连 +20%。 |
