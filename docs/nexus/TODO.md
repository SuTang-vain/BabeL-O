...
| P1 | 子 Agent lifecycle / transcript / permission inheritance | [TODO_agents.md](./TODO_agents.md) | 子 Agent 已有正式 metadata、独立 child session transcript 引用、父队列任务摘要引用、permission inheritance 审计事件、父 session close 级联取消 smoke、单个 child cancel 结构化失败传播、session-scope approval 可配置继承 audit、远程 cancel/resume API，以及 provider-backed 非 dry-run deterministic AgentLoop smoke；下一步补真实 provider live/manual smoke 或 SDK/dashboard task API。 |
...
| P1 | TUI 编程工作流与视觉 smoke | [TODO_tui.md](./TODO_tui.md) | slash palette、permission panel 真实键盘路径、compact 工具渲染、input placeholder、read/edit/diff/Grep/Glob/task create 编程流和 resume session 已有 PTY smoke；下一步补 ask coding question、task update/status 入口，以及唯一输入框和 agent running 视觉 smoke。 |
...
| Agents / Optimize | Planner/Executor/Critic、subTasks、worktree isolation 已落地；TaskQueue 依赖失败传播已修复；structured-output repair 日志和 zodToJsonSchemaShape 已修复；子 Agent lifecycle metadata、transcriptPath 引用、permission inheritance 审计事件、父 session close 级联取消 smoke、单个 child cancel 结构化失败传播、session-scope approval 可配置继承 audit、远程 cancel/resume API 和 provider-backed 非 dry-run deterministic AgentLoop smoke 已落地。 | 真实 provider live/manual AgentLoop smoke，或 SDK/dashboard task API。 |
