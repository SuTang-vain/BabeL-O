...
6. **Watch / Stable Go TUI 维护**：`bbl go` 已提升为 `bbl chat` 的 stable opt-in alternative，源码/测试已按 `cmd/go-tui` + `internal/tui` 分层；后续守住 `bin/go-tui` 预编译入口、`go run ./cmd/go-tui` source fallback、`bbl go --check` 和 PTY smoke，不扩大 Go TUI 对 Nexus/runtime/context/AgentScheduler/provider/storage 的所有权。
...
| CLI / TUI | slash/tool palette、permission panel、boxed input、paste placeholder、层级渲染、compact/expanded 工具详情、PTY 基线、history search overlay ownership、长路径/CJK/ANSI/resize 视觉宽度、sub-agent running context gauge 回归、`/agents` 只读 multi-agent status view、LSP context mention、file attachment references、image reference metadata、opt-in vim mode、SessionChannel unread/link indicator / Inbox overlay、主对话关键事件卡片与真实 PTY smoke 已落地；关系可见化分层路线已进入 reference 规划。`bbl go` 已作为 stable opt-in alternative 验证 Nexus/WebSocket/permission/overlay 链路，并采用 `cmd/go-tui` / `internal/tui` 分层。 | 当前无打开的 P2 TUI 实现项；Go TUI 后续进入稳定维护，重点守住 source fallback、prebuilt packaging、PTY smoke 与 API parity；其他交互只在真实显示回归、dashboard/agent UX 需要、SessionChannel 关系可见化实现启动或发起侧 UX 明确时补专项项。 |
