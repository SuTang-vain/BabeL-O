...
| **P1** | **Module Coupling Governance** | [module-coupling-decoupling-and-re-aggregation-plan.md](./reference/module-coupling-decoupling-and-re-aggregation-plan.md), [active/TODO_cleanup.md](./active/TODO_cleanup.md) | 2026-06-18 补充 PR-sized execution map，并落地 Phase 0.5/1A/1B/2A/2B/2C/3A；Phase 3B+ 已完成 `ContextRefreshStrategy`、`ProviderTurnDriver`、`ToolDispatchPipeline`。Phase 4A+ 已新增 37 个 router 文件，并把 execute/stream 共享依赖拆成窄模块：`ActiveExecutionRegistry` / `ActiveExecutionLease` 收敛 active execution state 与 cleanup handle，`ExecutionTimeoutEvents` 收敛 timeout event/watcher machinery，`ExecutionTimeoutControls` 收敛 HTTP/WS timeout controls setup，`ExecutionPreparation` 收敛 request schema / timeout decision / cwd-session 初始化，`ExecutionFinalization` / `ExecutionSettlement` 收敛 session finalization / outcome / recovery metadata / summary settlement，`ExecutionEventProcessing` / `ExecutionEventSink` 收敛 soft watchdog decoration、event push/storage append、`execution_metrics` → NexusMetrics、BehaviorMonitor ingest 与 `cache_health` 派生/持久化，`RuntimeMetricsSnapshot` 收敛 `/v1/runtime/metrics` 聚合，`ExecutionHttpResult` 收敛 HTTP `/v1/execute` result envelope / status-code assembly，`ExecutionRuntimeOptions` 收敛 HTTP/WS 共享的 `runtime.executeStream()` options assembly，`ExecutionWebSocketControl` 收敛 WebSocket JSON parse / open-only send / `permission_response` 快路径、processed event forwarding、client-close tracker 与 timeout/summary event sender，`ExecutionStreamLoop` 收敛 HTTP/WS 共享 event loop / near-timeout checkpoint / terminal tracking；此前已覆盖 runtime/config/memory/models/metrics/provider/loop/skill/session read/wait/children/inspection/create/task-read/task-mutation/resume/permission/input/close/context/compact/cancel/agent/schema/working-set REST/tools/session-channel/task mutation/working-set observe/context observe 等 route clusters。`app.ts` 当前 864 lines，行数只作为累计健康指标。下一步剩余路线仍是 `/v1/execute` 与 `/v1/stream`，必须按 `review-high-risk` 继续拆 execute/stream lifecycle / response/socket cleanup 边界并保持小切片。 |
