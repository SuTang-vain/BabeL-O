...
| **P3 / Long-term** | **Go TUI 实验重写** | [active/TODO_tui.md](./active/TODO_tui.md), [reference/go-tui-rewrite-plan.md](./reference/go-tui-rewrite-plan.md) | `clients/go-tui/` + `bbl go` 已完成 MVP 与手动 Nexus/WebSocket 权限 smoke；长期目标是把 Go 作为可选高质量交互客户端，改善布局、输入 owner、overlay、权限、tool/agent/context/session-channel 可见性。Go TUI 不替代 TypeScript Nexus/runtime/context/AgentScheduler/provider/storage，也不成为默认入口；下一步只建议先补 `BABEL_O_RUN_GO_TUI_SMOKE=1` gated smoke，稳定后再推进 event renderer parity。 |
| **Watch** | **Session Channel + Scoped Memory** | [active/TODO_runtime.md](./active/TODO_runtime.md), [reference/session-to-session-memory-channel-plan.md](./reference/session-to-session-memory-channel-plan.md) | `SessionChannel` + `SessionMessage` + Inbox MVP、CLI/TUI unread inbox / ack / handoff、AgentScheduler parent-child channel、project/user/channel scoped memory diagnostics、API→Inbox→Context 可行性回归与 Phase E governed memory candidate MVP 均已落地；`memory_candidate` 只生成 review-only governance metadata，默认 `autoWrite=false`，不做 raw transcript sharing、不把跨 session message 当成直接指令、不启用完整 background dreaming 或高影响项目事实自动写入。 |
...
6. **P3 / Long-term Go TUI 实验重写**：`clients/go-tui/` MVP、`bbl go` launcher 与手动 Nexus/WebSocket permission smoke 已验证可行；短期只补 gated smoke 守住输入、permission_request、permission_response、tool_completed、result 链路，不扩大默认入口。Go TUI 只作为交互客户端，不迁移 Nexus/runtime/context/AgentScheduler/provider/storage；promotion gate 详见 reference 规划。
...
| [reference/go-tui-rewrite-plan.md](./reference/go-tui-rewrite-plan.md) | `bbl go` / Go TUI 长期实验重写规划；Go 只作为交互客户端，不拥有 Nexus/runtime/context/permission。 |
