...
| **P2 Plan** | **Context CWD Drift & Recall Governance** | [context-cwd-drift-and-recall-governance-plan.md](./reference/context-cwd-drift-and-recall-governance-plan.md) | 2026-06-17 升级为 Active Plan。**Phase A + Phase A Follow-up + Phase B + Phase C 已收口（2026-06-18）**：`extractAbsolutePaths()` 重写（URL guard + 实存绝对路径短路 + 多层 prose guard 覆盖 bare Latin word / CJK + em-dash / CJK + Latin 混排 / CJK 两段 prose；`isCjkOrCommonBasename` 取代 Han-only regex；SPACE_MARK 哨兵保留 escaped-space 路径），`createDefaultToolRegistry({ storage })` 新增 storage 哨兵（`storage: null` 隐藏 `contextSearch` / `contextRecent` / `contextSummarize`），3 个调用点（`createRuntime.ts` / `AgentScheduler.ts` / `runnerComparisonBenchmark.ts`）全部更新；`SessionRootContinuity` 纯 decision helper（4 decision × 9 reason）落地在 `src/runtime/sessionRootContinuity.ts`，`SessionRootContinuityEventSchema` 加入 `NexusEventSchema` discriminated union，`LLMCodingRuntime` 在 `hasSessionContext` 时调 `resolveCwdWithContinuity()` 并 yield `session_root_continuity` event，`AgentTrace` 投影把最近一条 continuity event 上浮为 run span 的 6 个 `lastContinuity*` attributes；累计 `test/system-prompt-builder.test.ts` 28→35 + `test/runtime-context-tools-registry-gate.test.ts` 4 个 + `test/session-root-continuity.test.ts` 17 个 + `test/runtime.test.ts` +6（5 Phase B + 1 URL-heavy regression）+ `test/inspect-session.test.ts` +1 共 **29 个新 test** 守住 `session_cf361f04` 全部失败点 + cwd 切换面。Phase D（`ContextEstimateCalibration` diagnostic）/ E（`ROOT_SCAN_REQUIRES_CONFIRMATION` 工具层 guard）/ F（`UserArtifactContinuity`）仍 Open，等真实 regression 触发再推进。 |
