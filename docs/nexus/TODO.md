...
| **P1** | **Go TUI `/model` 模型持久化** | [active/TODO_tui.md](./active/TODO_tui.md), [reference/go-tui-model-persistence-plan.md](./reference/go-tui-model-persistence-plan.md) | Phase 1 状态下 Go TUI `/model` Step 4 提交只改 `m.modelID` 内存字段（`clients/go-tui/internal/tui/tui.go:2135-2144`），不写 server、不落盘；`bbl go` 重启即丢失，操作员必须切回 `bbl config use` 或 `bbl chat /model` 才能落盘。规划路径：扩展 `POST /v1/runtime/config/select` 接受 `model` 字段（与 `profile` 互斥）+ TUI 端 Step 4 提交 state machine（in-flight 锁 + apply config）。不触碰 §5 路径 C 阶段 2 既有的 `setActiveProfile` 路径，不动 `bbl chat` / `bbl config use` 行为，不触碰 provider role defaults / auto model selection 延后项。`ConfigManager.resolveSettings()` 的 `profile.model > defaultModel` 优先级是既有 server 行为，UX caveat 在 reference doc §7 文档化。 |
...
6. **Watch / Stable Go TUI 维护**：`bbl go` 已提升为 `bbl chat` 的 stable opt-in alternative，源码/测试已按 `cmd/go-tui` + `internal/tui` 分层；后续守住 `bin/go-tui` 预编译入口、`go run ./cmd/go-tui` source fallback、`bbl go --check` 和 PTY smoke，不扩大 Go TUI 对 Nexus/runtime/context/AgentScheduler/provider/storage 的所有权。同时开启 Phase 10 规划，探讨将 Go TUI 作为面向用户分发的正式版，TS TUI 简化为开发调试控制台的演进规划。**P1 打开项**：Go TUI `/model` Step 4 模型持久化（按 [reference/go-tui-model-persistence-plan.md](./reference/go-tui-model-persistence-plan.md) 推进协议 + 客户端切片），收口前不进其他 TUI 实现项；UX caveat `activeProfile.model > defaultModel` shadow 显式文档化在 reference doc §7。
...
| [reference/go-tui-model-persistence-plan.md](./reference/go-tui-model-persistence-plan.md) | Go TUI `/model` Step 4 模型持久化规划：扩展 `POST /v1/runtime/config/select` 接受 `model` 字段 + TUI Step 4 提交 state machine（in-flight 锁 + apply config）；不动 §5 路径 C 阶段 2 既有的 `setActiveProfile` 路径、不动 `bbl chat` / `bbl config use` 行为、不动 `resolveSettings()` 优先级。 |
