...
| **P1** | **Go TUI 任务面板实时更新 + AskUserQuestion 弹窗** | [proposals/go-tui-task-board-and-ask-user-question-plan.md](./proposals/go-tui-task-board-and-ask-user-question-plan.md), [active/TODO_tui.md](./active/TODO_tui.md) | TaskCreate 工具不发出 task_created 事件 → Go TUI 任务面板在 turn 中不更新；AskUserQuestion 从未实现。Phase 1-7 逐步修复：事件发射、Go TUI 事件处理、事件协议、工具 + HTTP 路由、TUI 弹窗、验证。 |
