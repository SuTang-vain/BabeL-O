...
| **P1** | **Intake 授权连续性与保守性修复** | [proposals/authorization-continuity-execution-plan.md](./proposals/authorization-continuity-execution-plan.md), [proposals/intake-conservatism-analysis.md](./proposals/intake-conservatism-analysis.md), [active/TODO_runtime.md](./active/TODO_runtime.md) | **证据：`session_1de7cf54`（Turn 6 "继续任务" 授权重置 → TOOL_DENIED）**。Intake 层三层过度防御：兜底逻辑偏保守（`requiresTools ? 'inspect' : 'none'`）、`isLocalChangeAuthorizationRequest()` 正则遗漏常见措辞（"继续"、"修复"、"可写模式"）、无历史授权状态继承。**修复规划**：Phase 0-4（约 5 天），包括延续指令继承逻辑 + 正则扩展 + Session 级别授权持久化 + replay 测试。设计原则从"默认拒绝，显式授权"调整为"上下文感知，合理推断，关键操作显式确认"。 |
