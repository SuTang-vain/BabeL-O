...
- [ ] **P1/P2: Model Fallback 执行入口**。与 provider registry role routing 联动，在用户显式确认后执行降低 max output、切换大上下文模型或切换 provider/profile；保持 `allowSilentModelSwitch=false`，并作为真实 provider live/manual AgentLoop smoke 后的下一项 provider 稳定性工作。
