...
- [x] 已完成 TUI 显示稳定性优化：ANSI/CJK 宽度计算、无外框 welcome header、启动 session banner 精简、spinner 清理、assistant streaming 合并、工具状态残影修复。
- [x] 已完成固定输入框第一版：长路径输入使用单行 viewport 截断，autosuggestion 仅在可容纳时显示，刷新时按旧内容和当前列宽清理 wrapped rows，避免 resize 后自动换行残影和重复 prompt。
- [x] 已完成输入框视觉 polish、ghost hint 与发送态修复：主输入改为上下分隔线 boxed prompt、裸 `>` 输入行和底部 `? for shortcuts` + 当前模型状态；分隔线使用安全宽度避免终端自动折行，空白 Enter 不提交空 turn；发送后按 readline 已换到下一行的真实光标位置清理整个输入框并以紫色文本渲染用户消息，不残留上分隔线、输入框 chrome 或提示尾巴。
...
- [x] 已建立最小 PTY smoke harness：覆盖 slash palette、permission panel 真实键盘路径、session approval cache、editable rule、reject instruction 和 compact Read 工具渲染。
- [x] 已支持 PTY resume session smoke：真实启动 chat、读取文件、退出、用实际 session id 恢复，并验证历史工具记录重绘。
- [x] 已核对 task update/status 可测入口：当前 local chat 只暴露 `task <title>` -> `TaskCreate`，task update/status 仍需新增 TUI/local command 或走 service/API smoke。

## 下一轮优先级

1. **补齐剩余编程闭环 smoke**：继续覆盖 ask coding question about files；task update/status 需先补 TUI/local command 或改走 service/API smoke。
2. **终端视觉回归 smoke**：在 PTY harness 上继续覆盖唯一输入框、overlay 打开、权限弹窗、agent running、工具运行时的渲染稳定性。
3. **MCP tool/resource display**：完成 P1 剩余的 MCP 工具和资源展示，使 MCP 能力在 TUI 中可发现、可审计。

> 总控排序：TUI 编程工作流补齐排在真实 provider live/manual AgentLoop smoke、SDK/dashboard query API、provider role defaults/fallback 之后。
...
- [x] TUI renderer/input 回归：工具参数摘要、工具完成原地更新、展开提示去重、assistant delta 合并、空白 assistant delta 过滤、固定输入框、发送态紫色消息、ANSI/CJK 宽度、permission panel 键盘决策、唯一 input owner 状态均有单元覆盖。
- [x] 最小 PTY smoke：slash palette、permission panel 真实键盘路径和 compact 工具渲染已有可选回归脚本覆盖。
- [x] 输入框 placeholder PTY smoke：覆盖空白 Enter 不提交、首字符输入后 ghost hint 清除且 prompt 保留。
- [x] PTY 编程工作流 smoke：read 文件、edit 并展开 diff、Grep/Glob 真实临时 git repo、TaskCreate 均已覆盖。
- [x] PTY resume session smoke：恢复真实 session id 后重绘历史工具记录已覆盖。
- [ ] ask coding question about files。
- [x] edit file and render diff。
- [x] deny risky command and continue。
- [x] approve risky command once and continue。
- [x] use Grep/Glob on a real repo。
- [x] create task。
- [ ] update/status task（local chat 暂无直接命令入口；需新增 TUI/local command 或走 service/API smoke）。
- [x] resume session。
...
- [x] permission panel PTY 回归：once/session/reject/reject feedback/editable rule 的真实键盘路径均可完成，Esc/Backspace 不会误批准。
