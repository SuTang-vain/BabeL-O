...
- [x] 已完成输入框视觉 polish 与 ghost hint 修复：主 prompt 使用紧凑 `chat` 标签，空输入 placeholder 只在真正空输入时显示；首字符输入会保留完整 prompt 并清除 hint，空白 Enter 不提交空 turn。
...
- [x] TUI renderer/input 回归：工具参数摘要、工具完成原地更新、展开提示去重、assistant delta 合并、空白 assistant delta 过滤、固定输入框、ANSI/CJK 宽度、permission panel 键盘决策、唯一 input owner 状态均有单元覆盖。
