...
## 已收口 Go TUI Running Interrupt / Queued Prompt

Go TUI agent running 态已支持两类输入：普通运行中可继续编辑输入框并按 `Enter` 预提交下一条用户指令，客户端只记录 `queuedPrompt`，不会启动并发第二条 stream；当前 turn 在 `result` / `error` / cancel completion 后自动启动 queued prompt。运行中首次 `Esc` 不直接报错或退出，而是打开黄色 permission-style guidance：`What should BabeL-O do instead?`，并预填 `BabeL-O should `；用户编辑后按 `Enter` 会请求取消当前 run 并把替代指令作为下一 prompt 排队；提示态再次 `Esc` 则无替代指令取消当前 run。取消请求优先走 Nexus `POST /v1/sessions/:sessionId/cancel`，失败时兜底关闭本地 WebSocket，避免 UI 卡死或异常退出。
