...
当前 P2 Advanced CLI/TUI 的唯一打开项为 SessionChannel Inbox overlay 真实 PTY smoke 补强；其他交互能力后续只在真实显示回归、PTY smoke drift 或新增交互状态时重新开未收口项。provider role defaults/fallback 仍按总控无限期 delay，不作为当前 TUI 前置项。
