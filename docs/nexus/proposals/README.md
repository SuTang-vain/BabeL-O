...
| [provider-stream-silent-hang-abort-propagation-plan.md](./provider-stream-silent-hang-abort-propagation-plan.md) | Partially Landed | The hard watchdog already fires, but the provider stream consumer only passively checks `signal.aborted` per delta, so a silent stream hangs for hours. Active `reader.cancel()` on abort (Phase 1) + single-source watchdog with log (Phase 2) landed; activeAgeMs observability (Phase 3) pending. Real session: `session_3c3ec27c` (8h hang) / `session_ffd44ccf`. |
