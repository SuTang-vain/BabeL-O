...
| [provider-stream-silent-hang-abort-propagation-plan.md](./provider-stream-silent-hang-abort-propagation-plan.md) | Draft | The hard watchdog already exists and fires, but the provider stream consumer only passively checks `signal.aborted` on each delta, so a silent stream hangs for hours. Active `response.body.cancel()` on abort + single-source watchdog + activeAgeMs observability. Real-session: `session_3c3ec27c` (8h hang) / `session_ffd44ccf`. |
