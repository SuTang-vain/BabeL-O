...
| --- | --- | --- |
| [adaptive-context-window-selection-plan.md](./adaptive-context-window-selection-plan.md) | Draft | Headroom-aware `selectRecentEvents`: preserve full history below the warning threshold instead of a fixed 4-turn cap every assembly; `recentTurnLimit` becomes a ceiling not a constant. Real session: `session_cd42cb65` (18%→3% drop per prompt on an 852k model). Compact-boundary threshold path untouched. |
