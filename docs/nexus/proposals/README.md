...
| [provider-stream-silent-hang-abort-propagation-plan.md](./provider-stream-silent-hang-abort-propagation-plan.md) | Partially Landed | The hard watchdog already fires, but the provider stream consumer only passively checks `signal.aborted` per delta, so a silent stream hangs for hours. Phase 1 (active `reader.cancel()` on abort) + Phase 2 (single-source watchdog with log) + Phase 3 (`activeAgeMs` deployed + false-positive reset) landed; optional BehaviorMonitor push detector deferred. Real session: `session_3c3ec27c` (8h hang) / `session_ffd44ccf`. |
