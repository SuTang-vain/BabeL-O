...
| *context-selection-dead-code-and-threshold-dedup-plan.md (graduated 2026-06-24)* | *was Draft* | *moved to [../reference/context-selection-dead-code-and-threshold-dedup-plan.md](../reference/context-selection-dead-code-and-threshold-dedup-plan.md) as `Active Plan`. Source-verified (2026-06-24) the dead `contextManager.ts` scoring scaffold (329 lines, 11 0-external-importer symbols, `score` field dropped at projection); the 4-site threshold duplication (tokenEstimator fallback 70/85 dead in real runtime path; cacheAwareCompactPolicy 70/90 + 80/93 in cache-preservation mode). Three Phase commits: Phase 1 remove dead scaffold; Phase 2 single `computeContextThresholds` source; Phase 3 consistency test. Per [../decisions/0001-documentation-lifecycle.md](../decisions/0001-documentation-lifecycle.md) §Decision, reference holds durable architecture, not "the next stage after proposals".* |
