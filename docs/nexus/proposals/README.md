...
| --- | --- | --- |
| [adaptive-context-window-selection-plan.md](./adaptive-context-window-selection-plan.md) | Draft | Headroom-aware `selectRecentEvents`: preserve full history below the warning threshold instead of a fixed turn+event cap every assembly; relax both `recentTurnLimit` and `recentEventLimit` at low usage. Real session: `session_cd42cb65` (1 of 11 turns retained at ~3% usage on an 852k model; `recentEventLimit=300` trips before a second turn fits). Compact-boundary threshold path untouched. Phase 0 closed. |
