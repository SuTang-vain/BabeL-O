...
| --- | --- | --- |
| [adaptive-context-window-selection-plan.md](./adaptive-context-window-selection-plan.md) | Partially Landed | Headroom-aware `selectRecentEvents`: preserve full history below the warning threshold instead of a fixed turn+event cap every assembly; relax both `recentTurnLimit` and `recentEventLimit` at low usage. Real session: `session_cd42cb65` (1 of 11 turns retained at ~3% usage on an 852k model → 11/11 after fix). Compact-boundary threshold path untouched. Phase 0/1 closed; Phase 2 (microcompact/snip gating) watch, Phase 3 (graduate to reference) open. |
