...
Behavior monitor 用于观察跨 session drift、loop、等待与提示注入等行为模式，帮助长任务在偏移前被发现。

### 边界

它不能替代 Nexus event、task scope、tool trace 或用户指令；任何 hint 都应作为诊断信号，而不是自动执行决策。

### 当前状态

本文属于 Partially Landed / Watch 文档。已落地与未落地项必须继续以 TODO、DONE、WORK_LOG 和源码验证为准。
