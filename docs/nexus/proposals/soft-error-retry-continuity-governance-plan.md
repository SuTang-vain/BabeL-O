...
> 2026-07-03 implementation — Slice 1 (B1 + B2-lightweight + B3) + Slice 3 C1-subset landed in PR #17. Slice 2-B landed in PR #18 (in-architecture partial-result strengthening). **C4 landed on branch `fix/soft-error-retry-c4-resume`**: when the user says "继续" right after a soft REQUEST_TIMEOUT, `contextAssembler` injects a resume nudge (`buildResumeFromSoftTerminal`) with the truncated in-flight tail, so the model continues from the cutoff instead of restarting; hard cancels never resume. Direction 2 (#15) graduated B1 into Tier 2 passthrough (B1 is now a Tier 2 special case). **Slice 2 A1/A2 deferred** — soft-timeout self-abort conflicts with the single-source watchdog architecture (executeStreamRoute.ts:126-137, guarded by execute-stream-watchdog.test.ts:121 + runtime.test.ts:6030). **A3 re-confirmed wrong** (breaks Phase 3 soft+0). **C2/C3 deferred** — C2 breaking event-type change, C3 new NexusEvent type. Full suite 1282/0; typecheck clean; `docs:check` 0; `format:check` 0.
...
2. **Slice 2-B (landed 2026-07-03, PR `fix/soft-error-retry-slice-2`): in-architecture partial-result strengthening.** Deep-tracing the timeout path revealed the original Slice 2 (A1 finish-window + A2 soft-timeout self-abort) **conflicts with the single-source watchdog architecture** — soft-timeout never aborts itself; termination is always the single watchdog (`executeStreamRoute.ts:126-137`, guarded by `execute-stream-watchdog.test.ts:121` and `runtime.test.ts:6030`). `startExecutionTimeoutControls` does not even hold the `timeoutController`, so soft self-abort is an architecture-level change. Rather than overturn single-source watchdog, Slice 2-B improves the watchdog-terminal path within it: `buildPartialTimeoutSummary` now prefers the **in-flight turn's** output (after the last `user_message`) and keeps the **tail** (where the cutoff happened), so a 95%-done answer truncated by the watchdog is preserved as the partial result instead of an early-turn prefix. **A1/A2 deferred** to a separate proposal that must justify overturning single-source watchdog. **A3 re-confirmed wrong** — breaks Phase 3's intentional `soft + maxSoftTimeoutExtensions:0` capability (`runtime.test.ts:6081`), and forcing ≥1 does not fix seq 924 (the 240s watchdog still fires during the extension).
