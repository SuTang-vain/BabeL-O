...
> 2026-07-03 implementation — Slice 1 landed (B1 + B2-lightweight + B3) on branch `fix/soft-error-retry-continuity`. B1: `self_diagnosis_request` exemption in `shouldSuppressToolsForIntent` (`intentGuidance.ts`). B2-lightweight: `severity: 'soft'` added to `TOOL_CALL_SUPPRESSED_BY_USER_INTENT` details — the event `type` stays `"error"` for now; the full type change is deferred to Slice 3 (C2) once the `severity` framework and consumer migration land. B3: suppression nudge rewritten so the model knows the retry will pass through. **A2 deferred to Slice 2** with a corrected design: `maybeDecorateWatchdogError` only decorates `watchdog.fired` (hard cutoff) errors, where `retryable: false` is correct — seq 924 was watchdog-fired (`kind:"watchdog"`), not soft-timeout-only. The soft-timeout retryable path must be built alongside A1's finish-nudge so soft-timeout-only is distinguished from watchdog. TDD: 2 new red tests (B1 in `runtime-llm.test.ts`, B2/B3 assertions in `runtime.test.ts`); full suite 1269/0; typecheck clean; `docs:check` 0; `format:check` 0.
...
1. **Slice 1 (landed 2026-07-03, directly unblocks seq 1024):** B1 (`self_diagnosis_request` exemption in `shouldSuppressToolsForIntent`) + B2-lightweight (`severity: 'soft'` on the suppression event; `type` unchanged, full type change deferred to C2) + B3 (nudge tells the model the retry will pass through). **A2 was descoped from Slice 1** — see the implementation note above: `maybeDecorateWatchdogError` only decorates `watchdog.fired` (hard cutoff) errors where `retryable: false` is correct; the soft-timeout retryable path moves to Slice 2 alongside A1's finish-nudge. Fully additive, no schema/type change.
2. **Slice 2:** A1 (finish nudge) + A3 (enforce `≥1` extension). Touches the timeout path; needs the soft-finish-window regression.
3. **Slice 3 (framework):** C1 (`severity` field) + C2 (soft signals stop emitting `type:"error"`) + C3 (soft budget exhaustion event) + C4 (resume from soft signal). Largest; lands after Slice 1/2 prove the pattern.
