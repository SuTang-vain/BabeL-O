...
1. **Slice 1 (MVP, directly unblocks seq 924/1024):** B1 (`self_diagnosis_request` exemption) + A2 (`retryable: true` for soft timeout) + B2/B3 (suppression event softening + nudge). Low risk, deterministic, no schema change beyond the new event type.
