...
| [architecture-optimization-assessment-plan.md](./architecture-optimization-assessment-plan.md) | Active Plan | Holistic architecture health assessment with optimization targets not already owned by existing Active Plans: SkillsProvider abstraction (T1), agent loop decomposition (T3), context assembler step decomposition (T4), and error handling consistency (T2). Does not duplicate coupling, layer-direction, tool-permission, or agent-maturity plans. |
