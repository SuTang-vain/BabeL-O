...
| [long-running-context-assembly.md](./long-running-context-assembly.md) | Working set, resume, context assembly, REST/CLI/WS interfaces, and Nexus-owned long-running session state. | Active Plan. R0 / R1 / R2 / R3 / R4 / R5 / R6 / R7 all closed as of 2026-06-20; runtime `executeStream` hot path now injects persisted Nexus working set as authoritative active context, and `/v1/context/observe` streams redacted assembled frames in real time. Replay Gate (`test/r7-replay-gate.test.ts` 15/15) verifies the three original failing sessions. ADR-5 retired 2026-06-20: `natural_pause` decision branch + `BABEL_O_NATURAL_PAUSE_SUPPRESS` env flag removed; 5-reason contract collapsed to `disabled / duplicate_turn / growth_threshold / forced / insufficient_signal`; signal coverage now lives in working set + behaviorTrace (`trajectory-end` / `user-redirect`); INV-11 governance fence retained in 5 sites. Bug 1.1 closed 2026-06-20: 4th on-demand tool `contextSessions` added for cross-session metadata search (`storage.listSessions()` projection); single-session event tools (`contextSearch` / `contextRecent` / `contextSummarize`) keep their orthogonal scope. Bug 1.2 closed 2026-06-20: `tool.riskForInput` extended to `{ kind, rule? }` rich shape; `bashTool.riskForInput` returns classifier rule; deny message appends `(classifier: <rule>)` so the model sees WHY a Bash command was rejected. Bug 1.3 closed 2026-06-20: `contextRecent` defaults to excluding `hook_started` / `hook_completed` / `usage` / `thinking_delta` / `assistant_delta` / `tool_completed` so the model only sees user-visible events by default; caller-supplied `excludeEventTypes` merges on top of default rather than replacing it. Bug 2 closed 2026-06-21: `redactContext` summary mode now also strips `systemPromptBlocks[].text`; `RedactedContextBlock = Pick<...SystemPromptBlock, 'cacheable'>` narrows the type so observers only get length/cacheable metadata, not verbatim prompt text. |
...
| [behavior-monitor.md](./behavior-monitor.md) | Behavior trace, cross-session diagnostics, live hints, and Go loop visualization follow-up. | Treat behavior trace as diagnostics and live guidance, not as memory or authoritative task state. |
...
| --- | --- | --- |
| Runtime hot-path persisted working-set injection | [long-running-context-assembly.md](./long-running-context-assembly.md) | Closed (R2, 2026-06-18). `LLMCodingRuntime.executeStream()` loads/updates `PersistedWorkingSetTracker` via `loadWorkingSetOverride.ts` (read) + `applyWorkingSetUpdate.ts` (write) and passes `workingSetOverride` to every context refresh; `test/runtime-working-set-hot-path.test.ts` 7/7. |
| REST `PUT /v1/context/working-set/:sessionId` and `/v1/working-set/observe` shared tracker | [long-running-context-assembly.md](./long-running-context-assembly.md) | Closed (R3, 2026-06-18). PUT goes through `WorkingSetBroadcaster.mutate(cwd, fn)` so the per-cwd tracker is shared with `/v1/working-set/observe` subscribers; `test/r3-rest-put-observe.test.ts` 6/6. |
| WebSocket `/v1/context/observe` with real runtime assembled-context events | [long-running-context-assembly.md](./long-running-context-assembly.md) | Closed (R4, 2026-06-20). Real `executeStream` publishes redacted `assembled` frames via `ContextBroadcaster.publish()`; `redactContext(ctx, 'summary')` is default, `?full=1` opt-in verbatim; `test/r4-context-observe-runtime-e2e.test.ts` 9/9. |
...
| Long-term memory section through a MemoryProvider contract | [long-running-context-assembly.md](./long-running-context-assembly.md), [memory-governance-plan.md](./memory-governance-plan.md) | Deferred until memory governance is stable. |
| Go loop mirror for behavior monitor visualization | [behavior-monitor.md](./behavior-monitor.md) | Client follow-up. |
