...
| [long-running-context-assembly.md](../proposals/long-running-context-assembly.md) | Working set, resume, context assembly, REST/CLI/WS interfaces, and Nexus-owned long-running session state. | Treat as a partially landed implementation line; remaining items are API and observe-stream follow-up, not a full rewrite. |
| [context-cwd-drift-and-recall-governance-plan.md](./context-cwd-drift-and-recall-governance-plan.md) | Regression plan for prompt-derived cwd drift, provider usage calibration, and storage-backed session recall tools. | Treat as Active Plan; use it when context drift starts from cwd/path extraction, broad root scans, or failed `contextSearch` / `contextRecent` recall. Phase A + Phase A Follow-up (2026-06-18, 5-class prose guard + URL guard + storage sentinel) and Phase C (2026-06-17) closed; Phase B / D / E / F Open. |
...
| --- | --- | --- |
| REST `PUT /v1/context/working-set/:sessionId` | [long-running-context-assembly.md](../proposals/long-running-context-assembly.md) | Gated write operation. |
| WebSocket `/v1/context/observe` with assembled-context events | [long-running-context-assembly.md](../proposals/long-running-context-assembly.md) | Runtime integration follow-up. |
| CWD drift and session recall regression corpus | [context-cwd-drift-and-recall-governance-plan.md](./context-cwd-drift-and-recall-governance-plan.md) | Active Plan; Phase A + Phase A Follow-up (2026-06-18, prose guard + URL guard + storage sentinel) and Phase C (2026-06-17) closed; Phase B / D / E / F Open, gated on future real-session regressions. |
...
优先收口 `context observe`、working-set 写接口、真实回归样本和 Go loop 可视化；长期记忆能力继续跟随 Memory governance，避免把记忆、行为诊断和上下文事实混为一体。
