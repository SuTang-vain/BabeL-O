...
> Priority: P0 — real-session regression: `contextSearch` returned empty across a long session, forcing the model into a multi-retry loop that still missed the newest user messages. Phase 0/1 closed 2026-06-21.
> Source of truth: [../TODO.md](../TODO.md), [../active/TODO_runtime.md](../active/TODO_runtime.md), [../WORK_LOG.md](../WORK_LOG.md), `src/tools/builtin/contextSearch.ts`, `src/tools/contextTools.ts`, `src/storage/Storage.ts`, `src/storage/EventRepository.ts`, `src/storage/MemoryStorage.ts`, `test/contextTools.test.ts`, `test/storage-event-repository.test.ts`, `scripts/repro-context-search-260621.mjs`
> Governance: Indexed by [README.md](./README.md). Canonical owner of "the `contextSearch` retrieval algorithm and its storage boundary". On-demand tool semantics stay in [long-running-context-assembly.md](./long-running-context-assembly.md) §7.1; storage interface segregation stays in [storage-interface-segregation-reference.md](./storage-interface-segregation-reference.md); recoverable-failure tool behavior stays in [runtime-tool-loop-governance-plan.md](./runtime-tool-loop-governance-plan.md); PR review level for this change is `review-high-risk` per [development-process-stability-governance-plan.md](./development-process-stability-governance-plan.md) §6.1.
> Related: [long-running-context-assembly.md](./long-running-context-assembly.md), [storage-interface-segregation-reference.md](./storage-interface-segregation-reference.md), [runtime-tool-loop-governance-plan.md](./runtime-tool-loop-governance-plan.md), [development-process-stability-governance-plan.md](./development-process-stability-governance-plan.md), [context-cwd-drift-and-recall-governance-plan.md](./context-cwd-drift-and-recall-governance-plan.md)
...
| Phase 3 | Closed 2026-06-21 | Graduate this proposal to `reference/` as `Active Plan` once Phase 1 is closed against the regression session; summarize implementation into `DONE.md` / `WORK_LOG.md`. | Document lifecycle move verified by `npm run docs:check` (failureCount 0); `reference/README.md` updated; `proposals/README.md` carries the graduation note. |
