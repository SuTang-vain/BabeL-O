...
> Status: **Stable alternative to `bbl chat` (promoted 2026-06-10 via Phase 9)**
> Current baseline: `clients/go-tui/` standard Go layout (`cmd/go-tui` executable + `internal/tui` core package/tests) + `bbl go` launcher (6 read-only overlays, 14 slash commands, permission panel, multi-path prebuilt install, `bbl go --check` readiness diagnostic)
> Production default: TypeScript `bbl chat` (unchanged; the Go TUI is opt-in)
...
- `clients/go-tui/` 使用 Bubble Tea / Bubbles / Lip Gloss / gorilla/websocket，并已拆为标准 Go layout：`cmd/go-tui/` 负责 executable entry/flags/env/exit，`internal/tui/` 负责核心 TUI model、Nexus client helper、event rendering、overlay 和白盒状态机测试。
- `bbl go` 入口已接入 CLI；优先运行本地 `clients/go-tui/bin/go-tui` 二进制，缺失时 fallback 到 `go run ./cmd/go-tui`。
...
`bbl chat` 继续是生产默认入口。Go TUI 已在 2026-06-10 通过 Phase 9 promotion gate，成为 stable opt-in alternative；以下条件作为后续维护守门标准，而不是待提升前置条件：
...
- Model/config wizard 暂不复刻完整 TypeScript 交互；优先提供共享配置显示、受限 profile 切换与跳转提示。

收口标准：

- 常用 slash commands 可发现、可补全、可执行或给出明确“暂未支持”提示。
- 不绕过 Nexus API。
- 不在 Go TUI 内直接读写 config 文件；profile 切换只能走 Nexus 受限 `POST /v1/runtime/config/select`，更复杂的 model/role/provider mutation 仍需独立 API、确认 UI 与治理策略。
...
- 不把本机 `clients/go-tui/bin/go-tui` 直接提交仓库。
- 评估 release asset：
...
### Phase 10 — Future Strategic Vision: Go TUI as Production Default, TS TUI as Dev-only Playground

**目标**：将 Go TUI (`bbl go`) 升级为正式面向终端用户的官方发布版本，而 TypeScript TUI (`bbl chat`) 逐步退居幕后，作为面向开发贡献者（Core Contributors）的原型开发与调试工具。

**演进动因**：
1. **分发体验与零依赖（Zero-Dependency）**：Go TUI 编译后是 ~10MB 的单文件，无需任何本地 Node.js 运行时或巨大的 `node_modules` 缓存即可独立分发给最终用户；而 Nexus 服务端作为 Headless 后台，可以被打包并由 Go 客户端静默拉起。
2. **交互稳定性（UX Robustness）**：Bubble Tea 能够彻底解决 Node.js readline 在光标定位、折行 reflow 和窗口 resize 时的各类闪烁与崩溃缺陷，提供企业级的交互质感。
3. **开发效率（Developer Velocity）**：核心开发和协议测试保持在 TypeScript Monorepo 中（即运行 `bbl chat dev` ），方便快速验证 API，避免频繁跨编译的开销。

**演进任务**：
- **入口合并**：未来正式发布版中，全局命令 `bbl` 默认指向编译好的 Go TUI 二进制，提供 `--check` 及无感知后台 Nexus 自启动管理。
- **职责简化**：将 `src/cli/` 下的 TS TUI 简化，使其定位偏向“调试控制台（Debug Console）”，不再追求复杂的 TUI 浮层，把渲染开发重心完全迁移至 Go TUI。
- **协议生成自动化**：探索基于 Zod 到 Go Struct 的自动化转换，减少由于接口变动带来的双端结构体同步开销。

## 风险与治理

| 风险 | 影响 | 治理 |
| --- | --- | --- |
| 双 TUI 维护成本升高 | 两套交互路径分叉 | Phase 1-8 收口后于 2026-06-10 经 Phase 9 promotion gate 决策保持双 TUI 并存：Go TUI 为 stable alternative（`bbl go` opt-in），`bbl chat` 仍为默认；共享 Nexus 协议 + 公共测试。详见 `docs/nexus/PHASE_9_DECISION.md`。 |
...
当前 Phase 1-9 均已收口，后续进入稳定维护：

1. 守住标准目录布局：`cmd/go-tui` 只放 entrypoint，`internal/tui` 保留核心 package 与白盒测试，`bin/` 只放本地构建产物。
2. 守住启动路径：`bbl go` 优先 `clients/go-tui/bin/go-tui`，缺失时 fallback `go run ./cmd/go-tui`。
3. 新增 UI/overlay/API parity 前，先补对应 Go unit test 与 gated PTY smoke。

不把 Go TUI 扩展成 runtime/provider/context 所有者；需要新能力时优先补 Nexus API/event schema，再由 Go TUI 消费。
