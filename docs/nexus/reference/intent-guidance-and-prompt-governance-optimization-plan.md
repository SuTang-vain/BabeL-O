...
用户指令跟随漂移往往来自意图误判、respond-only 误归类、工具可见性缺失或 prompt 与 runtime policy 不一致。

### 边界

模型第一回复可以帮助引导，但不应替代 deterministic policy。是否需要工具、是否需要验证、是否可执行，必须由 runtime policy 和证据规则守住。

### 当前状态

本文作为 Partially Landed 参考保留。新的 prompt 调整必须能解释真实 session regression，并同步到 prompt-model-governance-index。
