...
> Source of truth: [../TODO.md](../TODO.md), [../active/TODO_runtime.md](../active/TODO_runtime.md), [../active/TODO_cleanup.md](../active/TODO_cleanup.md), [../DONE.md](../DONE.md), [../WORK_LOG.md](../WORK_LOG.md), `src/nexus/`, `src/runtime/`, `src/shared/`, `src/tools/`, `src/storage/`, `src/providers/`, `scripts/audit-dependency-boundary.js`, `scripts/audit-coupling.js`, `scripts/audit-layer-direction.js`
> Governance: Canonical coupling governance entry point. Indexed by [README.md](./README.md) and cross-referenced from [development-process-stability-governance-plan.md](./development-process-stability-governance-plan.md), [context-governance-index.md](./context-governance-index.md), [tool-governance-plan.md](./tool-governance-plan.md), [agent-runtime-architecture-maturity-plan.md](./agent-runtime-architecture-maturity-plan.md), [context-cwd-drift-and-recall-governance-plan.md](./context-cwd-drift-and-recall-governance-plan.md), [layer-direction-audit-enforcement-plan.md](./layer-direction-audit-enforcement-plan.md), [storage-interface-segregation-reference.md](./storage-interface-segregation-reference.md).
> Related: [runtime-tool-loop-governance-plan.md](./runtime-tool-loop-governance-plan.md), [memory-governance-plan.md](./memory-governance-plan.md), [prompt-model-governance-index.md](./prompt-model-governance-index.md), [evidence-governance-index.md](./evidence-governance-index.md), [go-client-distribution-governance-index.md](./go-client-distribution-governance-index.md), [long-running-context-assembly.md](./long-running-context-assembly.md), archived source documents in [../archive/](../archive/).
...
The line-count targets in this document are health indicators, not single-PR merge gates. `LLMCodingRuntime <= 600 lines` and `app.ts <= 400 lines` remain useful north-star signals, but no implementation PR should attempt to reach them in one large refactor. For `LLMCodingRuntime`, the real stopping condition is structural: once the main loop boundary is reviewable, complex decisions are delegated to focused modules with tests, and the class reads as orchestration rather than business logic, the project should stop splitting it purely for line-count reduction. The actual execution path is the PR-sized phase map in [Execution Slicing Addendum](#execution-slicing-addendum).

**Runtime decomposition stop rule (2026-06-21)**: `LLMCodingRuntime <= 600 lines` is no longer a hard completion requirement. Phase 3B+ may close above 600 lines if the runtime loop has explicit orchestration boundaries, provider recovery / context refresh / tool dispatch / working-set / behavior-trace / hook or compact decisions are isolated behind tested helpers or strategy objects, and further extraction would mostly hide sequencing rather than reduce coupling. When this condition is met, new runtime decomposition PRs must cite a concrete coupling or regression risk, not only a lower line-count goal.

**Stream B stop rule (2026-06-21)**: Phase 2 closes when the `RuntimeServices` container (Phase 2D) lands and the remaining module-level singletons (`defaultEverCoreRuntimeManager`, legacy `ConfigManager.getInstance()` callsites, `defaultContextBroadcaster` 7 refs) collapse into one typed container constructed once in `createDefaultNexusRuntime`. Until 2D lands, `npm run coupling:audit` is informational for these singletons.
...
**Stream G — SqliteStorage Repository Decomposition** closed on 2026-06-21. The 1753-line `SqliteStorage.ts` monolithic database manager has been fully decomposed: 8 repository classes own per-table operations, with `SqliteStorage.ts` now **968 lines (-785 / -44.8%)** acting as a thin-delegation facade. Landed (3B-20 → 3B-27): `EventRepository` (206 lines, events + sequence allocation + duplicate-repair + tool-trace / execution-metrics callbacks via constructor-injected options), `TaskRepository` (166 lines, tasks + JSON-column serialization), `AuditRepository` (131 lines, permission_audits), `ToolTraceRepository` (210 lines, tool_traces + composite `started_at | tool_use_id` cursor pagination), `SessionChannelRepository` (316 lines, session_channels + session_messages + inbox filter + read-modify-write acknowledge), `AgentJobRepository` (145 lines, agent_jobs + dual storage `job_json` + indexed filter columns), `ExecutionMetricsRepository` (243 lines, execution_metrics + 36 columns + boolean 0/1 encoding), `LoopPaneRepository` (181 lines, loop_state + read-modify-write `updateLoopPaneRev`). Phase 9 row is updated to "Closed 2026-06-21" with this full eight-slice status recorded below.

**Post-Stream-G extension (canonical ownership delegated)**: Per-domain `Repository<T>` sub-interfaces and polymorphic in-memory backends are owned by [storage-interface-segregation-reference.md](./storage-interface-segregation-reference.md). Layer-direction enforcement (`cli → nexus` / `cli → runtime` / `shared → providers` allowlist) is owned by [layer-direction-audit-enforcement-plan.md](./layer-direction-audit-enforcement-plan.md). Both are `Active Plan` references that this doc cross-references rather than owns.
...
- `SqliteStorage.ts` is now a 968-line thin-delegation facade that owns only schema setup, transaction locks, public-method dispatch, and cross-cutting helpers (session row map, listAllEvents for SessionGetOptions). 8 repository classes own per-table operations: `EventRepository` (206 lines, events + sequence allocation + duplicate-repair + tool-trace / execution-metrics callbacks), `TaskRepository` (166 lines, tasks + JSON-column serialization), `AuditRepository` (131 lines, permission_audits), `ToolTraceRepository` (210 lines, tool_traces + composite cursor), `SessionChannelRepository` (316 lines, session_channels + session_messages + inbox filter), `AgentJobRepository` (145 lines, agent_jobs + dual storage), `ExecutionMetricsRepository` (243 lines, execution_metrics + 36 columns + boolean encoding), `LoopPaneRepository` (181 lines, loop_state + read-modify-write). Stream G closed on 2026-06-21 with `SqliteStorage.ts` 1753 → 968 lines (-785 / -44.8%). The storage domain is now decomposed into independently reviewable, independently testable units.
...
9. **`SqliteStorage` was a monolithic database manager.** The single class `SqliteStorage.ts` handled WAL log batches, schema setup, session retrieval, task mutations, and event lists. Changes to different data models were coupled in this file, making testing of isolated entities harder and increasing merge conflict risk. Stream G closed on 2026-06-21 with 8 repository classes extracted: `EventRepository` (3B-20), `TaskRepository` (3B-21), `AuditRepository` (3B-22), `ToolTraceRepository` (3B-23), `SessionChannelRepository` (3B-24), `AgentJobRepository` (3B-25), `ExecutionMetricsRepository` (3B-26), `LoopPaneRepository` (3B-27). `SqliteStorage.ts` 1753 → 968 lines (-785 / -44.8%). Per-table operations are now independently reviewable, independently testable (67 storage tests + 28 R-gate tests cover the contract), and SQL UPSERT / WHERE / ORDER BY / JSON serialization behavior is byte-identical.
...
- `LLMCodingRuntime <= 600 lines` and `app.ts <= 400 lines` are cumulative health signals, not single-PR exit criteria.
- `LLMCodingRuntime <= 600 lines` is not a hard Phase 3 completion gate. If the main loop has clear orchestration boundaries and remaining inline code mainly preserves event / compact / permission sequencing, further splitting should stop unless a concrete coupling risk, regression, or testability gap justifies it.
...
- Reduce `LLMCodingRuntime` from 1758 lines / 33 imports into a reviewable orchestrator by extracting strategy objects and standalone helpers whose boundaries are testable independently. `≤ 600 lines / ≤ 12 imports` remains an aspirational health signal, but the actual goal is a readable orchestration shell with explicit decision boundaries, not line-count minimization for its own sake.
...
- Decompose the 1753-line `SqliteStorage.ts` class by extracting domain-specific database operations into separate repository classes. Stream G closed on 2026-06-21 with 8 repositories extracted: `EventRepository` (3B-20, 206 lines), `TaskRepository` (3B-21, 166 lines), `AuditRepository` (3B-22, 131 lines), `ToolTraceRepository` (3B-23, 210 lines), `SessionChannelRepository` (3B-24, 316 lines), `AgentJobRepository` (3B-25, 145 lines), `ExecutionMetricsRepository` (3B-26, 243 lines), `LoopPaneRepository` (3B-27, 181 lines). `SqliteStorage.ts` is now 968 lines (-785 / -44.8%) acting as a thin-delegation facade that owns only schema setup, transaction locks, public-method dispatch, and cross-cutting helpers.
...
| `nexus/routers/runtimeModelsRouter.ts` | 8 | 0 |
| `cli/commands/go.ts` | 8 | 0 |
| `cli/runSessionFlow.ts` | 7 | 0 |
| `nexus/runnerComparisonBenchmark.ts` | 5 | 0 |
| `nexus/createRuntime.ts` | 4 | 0 |
...
The current class has several intertwined responsibilities. Decomposition should extract decision-heavy responsibilities into standalone classes or helpers with focused interfaces, while preserving the top-level event / compact / permission sequencing where it is easier to audit inline. The goal is a reviewable orchestrator boundary, not maximum fragmentation.

| Responsibility | Current location (line range in `LLMCodingRuntime.ts`) | New class |
| --- | --- | --- |
| Loop orchestration | `runExecuteStreamInner` (284-1157) | Keep inline or move to `RuntimeOrchestrator` only if the extracted boundary remains easier to review than the current sequencing |
...
Phase 3B+ should stop once this shape is true: `LLMCodingRuntime` owns lifecycle ordering, event yielding, and the thin call graph between strategies; provider recovery, context refresh, tool dispatch, working-set read/write, behavior tracing, hook/compact/token-budget decisions, and provider-message translation are owned by focused modules with direct tests. A line count above 600 is acceptable at that point if the remaining code is primarily orchestration and extracting it would obscure ordering or create trampoline modules.
...
| Phase 6 Watch — Events / RuntimeEnv | Watch (re-evaluated 2026-06-21) | `shared/events.ts` exhaustive translator (Stream F, Phase 6 north-star). The Phase 7 half of this Watch row (RuntimeEnv) is **In Progress 2026-06-21** — see Phase 7 row above for the parser + 2D.1-2D.5 migration map. Original trigger "Defer until Phase 1-4 reduce hot spots" is now satisfied (Phase 1-4 all closed by 2026-06-19, Phase 9 closed 2026-06-21). Re-evaluation: Phase 6 (events codegen) remains Watch because the 333-line hand-written if/else in `src/runtime/eventsTranslator.ts` is reviewable today and the `Record<type, translator>` exhaustive-lookup migration is high-cost (~500-800 lines) for incremental value. | Phase 6 un-blocks when a new event type is added without translator coverage or a translator bug slips through review; Phase 7 lands progressively through the 2D.1 → 2D.5 PR map. |
| Phase 2D — RuntimeServices Container | In Progress 2026-06-21 (container landed, 7 migration PRs pending) | Introduce the typed composition-root container `RuntimeServices` so the remaining module-level singletons (`defaultEverCoreRuntimeManager`, legacy `ConfigManager.getInstance()` 22+ callsites, `defaultContextBroadcaster` 7 refs) collapse into one container constructed once in `createDefaultNexusRuntime`. **Landed 2026-06-21** (commit `8173de3`): `src/runtime/env.ts` (195 lines, `parseRuntimeEnv` boot-time snapshot — `RuntimeEnv = { nexus: NexusEnv, workspace: WorkspaceEnv }` with default fallback + parse errors mirroring `nexus/server.ts:parsePolicyMode` alias set + `parseBoolean` yes/no compatibility) + `src/nexus/services.ts` (114 lines, `createRuntimeServices` composition root in `src/nexus/` to keep `nexus → runtime` direction clean) + `test/runtime-env.test.ts` (17 cases) + `test/runtime-services.test.ts` (8 cases). `RuntimeServices` defaults fall back to legacy module-level instances (`ConfigManager.getInstance()`, `defaultEverCoreRuntimeManager`, fresh `ContextBroadcaster` / `ProviderSessionRules`, `parseRuntimeEnv(process.env)`) so legacy callers continue to work unchanged. `RuntimeServices` lives in `src/nexus/` (not `src/runtime/`) because it references Nexus-owned service classes — moving it to runtime would have created a `runtime → nexus` reverse import, contradicting Phase 1A + Phase 2B cleanup. **7 pending migration PRs** (2D.1 → 2D.7) are sequenced in the Phase 2 row above; each is a small PR preserving byte-identical behavior with a focused regression test and a before/after `npm run coupling:audit` fingerprint. | `test/runtime-env.test.ts` (17 tests) and `test/runtime-services.test.ts` (8 tests) pass; `node_modules/.bin/tsc --noEmit` is clean for the new files; `npm run coupling:audit --fail-on` still reports `runtimeToNexus: []` and `nexusToCli: []`; `npm run audit-layer-direction` 286 files / 1104 cross-module imports / 0 violations. 92/92 storage tests + 21/21 R-gate tests still pass (no regression; this slice is additive). Phase 2D itself lands when 2D.7 flips the Phase 2 row to "Closed 2026-06-XX". |
...
| --- | --- | --- | --- |
| Phase 0 | Closed 2026-06-18 | This document lands and the existing `npm run deps:audit` + `npm test` + `npm run build:smoke` are still green. | New canonical coupling reference is indexed from [reference/README.md](./README.md) and [TODO.md §Cleanup](../TODO.md); no source code is changed in this phase. Doc upgraded from "Active Plan" to canonical "Closed" 2026-06-21 after Stream G (Phase 9) closure, per the doc lifecycle split ADR-0001. |
...
| Phase 2 | In Progress 2026-06-21 (Open: 2D only, container landed 2026-06-21, 7 follow-up migrations pending) | Stream B singletons to `RuntimeServices`. 2A (2026-06-18 Closed) moved provider session rules to an injectable service; 2B (2026-06-18 Closed) removed the runtime hot-path dependency on `defaultContextBroadcaster`; 2C (2026-06-18 Closed) added explicit `ConfigManager` instances and migrated the first runtime / agent composition callsites. **2D container landed 2026-06-21** (commit `8173de3`): `src/runtime/env.ts` (195 lines, `parseRuntimeEnv` boot-time snapshot) + `src/nexus/services.ts` (114 lines, `createRuntimeServices` composition root) + 25 focused tests in `test/runtime-env.test.ts` (17 cases) + `test/runtime-services.test.ts` (8 cases). `RuntimeServices` lives in `src/nexus/` (not `src/runtime/`) to keep the layer direction clean: services.ts references Nexus-owned service classes (`ContextBroadcaster`, `EverCoreRuntimeManager`) which is a valid `nexus → runtime` direction, not a reverse import. Defaults fall back to legacy module-level instances so legacy callers continue to work unchanged. **2D follow-up PR map (each slice is one small PR per the one-PR = one-slice rule)**: **(2D.1)** `src/nexus/server.ts` 21 `process.env` reads → `services.env.nexus.*` (verification: `grep -rn 'process\\.env' src/nexus/` drops to 0 hits, satisfying Phase 7's verification criterion on line 459). **(2D.2)** `src/nexus/createRuntime.ts` 1 read + 1 `ConfigManager.getInstance()`. **(2D.3)** `src/cli/embedded.ts` 2 reads + 2 `ConfigManager.getInstance()` + 1 `defaultEverCoreRuntimeManager`. **(2D.4)** `src/cli/commands/go.ts` 8 reads + `src/cli/runSessionFlow.ts` 7 reads. **(2D.5)** `src/nexus/routers/runtimeModelsRouter.ts` 8 reads + `src/nexus/runnerComparisonBenchmark.ts` 5 reads. **(2D.6)** Remaining 22+ `ConfigManager.getInstance()` callsites in CLI commands / routers / runtime diagnostics / provider smoke / compact summary / bash tool. **(2D.7)** Phase 2 row → "Closed 2026-06-XX"; remove the 7 follow-up bullets from the plan doc; flip `[ ]` → `[x]` in `TODO_cleanup.md` Phase 2 entry. Each follow-up preserves byte-identical behavior with a focused regression test plus a before/after `npm run coupling:audit` fingerprint. | Focused service-injection tests prove no cross-talk for each migrated service; `npm test` passes when the full Stream B migration closes. Phase 2 closes when 2D.1 through 2D.7 land; no Stream B slice remains. |
| Phase 3 | In Progress 2026-06-21 | Stream C `LLMCodingRuntime` decomposition. Extract `ProviderTurnDriver`, `ContextRefreshStrategy`, `ToolDispatchPipeline`, `HookDispatcher`, and standalone decision helpers where they remove review risk. The class body should read as an orchestrator; the 4 callers continue to work without change. **Landed 2026-06-18 → 2026-06-20**: 3 strategy classes (`ContextRefreshStrategy` 56 lines, `ProviderTurnDriver` 63 lines, `ToolDispatchPipeline` 135 lines) + 5 standalone helpers (`eventsTranslator.ts` 332, `behaviorTraceTap.ts` 175, `loadWorkingSetOverride.ts` 118, `applyWorkingSetUpdate.ts` 83, `executeProviderRecoveryDecision.ts` 382 — 3B-19). `LLMCodingRuntime.ts` 1841 → 1493 (-348 / -18.9%) with main-loop `yield buildXxxEvent` (10) / `refreshRuntimeContextState` (6) / `compactSession` still inlined. | A new focused test per extracted class/helper exercises the boundary; the existing `test/runtime-llm.test.ts` and `test/runtime.test.ts` pass without modification; line count is tracked by `wc -l src/runtime/LLMCodingRuntime.ts` as a health signal. Phase 3 closes when the main loop audit shows remaining code is primarily orchestration; per doc stop rule (line 24 / line 233), the ≤ 600-line target is aspirational and Phase 3 may close above 600 lines if further extraction would only hide sequencing. |
...
| Phase 7 | In Progress 2026-06-21 (parser + tests landed 2026-06-21, migration 2D.1 → 2D.5 in plan) | Cross-cutting `process.env` consolidation. **Parser + tests landed 2026-06-21** (commit `8173de3`): `src/runtime/env.ts` (195 lines, `parseRuntimeEnv` boot-time snapshot) + `test/runtime-env.test.ts` (172 lines, 17 cases covering all NEXUS_*/BABEL_O_* env vars + error cases). `parseRuntimeEnv` returns a typed `RuntimeEnv = { nexus: NexusEnv, workspace: WorkspaceEnv }` snapshot; no live `process.env` reference is held. **Migration slices (per Phase 2 follow-up map)**: 2D.1 = `nexus/server.ts` 21 reads, 2D.4 = `cli/commands/go.ts` 8 reads + `cli/runSessionFlow.ts` 7 reads, 2D.5 = `nexus/routers/runtimeModelsRouter.ts` 8 reads + `runnerComparisonBenchmark.ts` 5 reads. The cross-cutting candidates `shared/config.ts`, `runtime/toolResultBudget.ts`, `runtime/sessionMemoryLite.ts`, `runtime/behaviorTrace.ts` are deferred to follow-up PRs after 2D.1 lands and proves the migration pattern on the largest file. | A new focused test `test/runtime-env.test.ts` parses a synthetic env and asserts every `RuntimeEnv` field is set correctly; `grep -rn 'process\\.env' src/nexus/` returns 0 hits once 2D.1 lands. |
...
| Phase 9 | Closed 2026-06-21 | Stream G Storage Decoupling closed with 8 repository classes extracted from `SqliteStorage.ts`. Landed (3B-20): `src/storage/EventRepository.ts` (206 lines, events + sequence allocation + duplicate-repair + sessions-table updated_at refresh + tool-trace & execution-metrics callbacks via constructor-injected options). Landed (3B-21): `src/storage/TaskRepository.ts` (166 lines, tasks + JSON-column serialization for `dependsOn` / `blocks` / `review` / `metadata`). Landed (3B-22): `src/storage/AuditRepository.ts` (131 lines, permission_audits + `approved` / `denied` decision enum). Landed (3B-23): `src/storage/ToolTraceRepository.ts` (210 lines, tool_traces + composite `started_at | tool_use_id` cursor pagination). Landed (3B-24): `src/storage/SessionChannelRepository.ts` (316 lines, session_channels + session_messages + cross-table inbox filter + read-modify-write acknowledge). Landed (3B-25): `src/storage/AgentJobRepository.ts` (145 lines, agent_jobs + dual storage `job_json` + indexed filter columns). Landed (3B-26): `src/storage/ExecutionMetricsRepository.ts` (243 lines, execution_metrics + 36 columns + `booleanToDb` / `dbToBoolean` 0/1 encoding). Landed (3B-27): `src/storage/LoopPaneRepository.ts` (181 lines, loop_state + read-modify-write `updateLoopPaneRev`). Net effect: `SqliteStorage.ts` 1753 → 968 lines (-785 / -44.8%) acting as a thin-delegation facade that owns only schema setup, transaction locks, public-method dispatch, and cross-cutting helpers. Per-table operations are now independently reviewable, independently testable, and SQL UPSERT / WHERE / ORDER BY / JSON serialization behavior is byte-identical. Stream G closed: no further per-table extraction is justified by a real boundary problem on this surface. **Next-stream extension**: per-domain `Repository<T>` sub-interfaces and polymorphic in-memory implementations — see [storage-interface-segregation-reference.md](./storage-interface-segregation-reference.md). | `wc -l src/storage/SqliteStorage.ts` is reduced 1753 → 968 (-785 / -44.8%); 67 storage focused unit tests pass (8 per-repository suites: `test/storage-event-repository.test.ts` 6 cases, `test/storage-task-repository.test.ts` 7 cases, `test/storage-audit-repository.test.ts` 6 cases, `test/storage-tool-trace-repository.test.ts` 8 cases, `test/storage-session-channel-repository.test.ts` 9 cases, `test/storage-agent-job-repository.test.ts` 10 cases, `test/storage-execution-metrics-repository.test.ts` 7 cases, `test/storage-loop-pane-repository.test.ts` 12 cases, plus existing `test/storage.test.ts` 2 cases as byte-identical regression evidence); session persistence remains intact (R2/R5/R7 + `npm run typecheck` + `npm run format:check` + `npm run docs:check` + `npm run deps:audit` + `npm run coupling:audit` `--fail-on` all green). |
...
- **Phase 3** — `wc -l src/runtime/LLMCodingRuntime.ts` is tracked as a health signal, not a hard gate. Per-class/helper focused tests must cover extracted boundaries; `test/runtime-llm.test.ts` and `test/runtime.test.ts` must pass. Phase 3 can stop above 600 lines when the remaining code is primarily orchestration and further splitting would obscure event / compact / permission sequencing.
...
- **Phase 7** — `grep -rn 'process\.env' src/nexus/` must report 0 hits once `parseRuntimeEnv` is in use: every env read goes through `services.env.nexus.*` / `services.env.workspace.*` / `services.env.runtime.*` / `services.env.feature.*`. `parseRuntimeEnv` itself lives under `src/runtime/env.ts` (not in `src/nexus/`). The 21 `process.env` reads currently in `src/nexus/server.ts` collapse to a single `parseRuntimeEnv(process.env)` call at boot.
...
3. **Stream C** — 已抽取 `ContextRefreshStrategy`、`ProviderTurnDriver`、`ToolDispatchPipeline` 三个 strategy class 与 `eventsTranslator`、`behaviorTraceTap`、`loadWorkingSetOverride`、`applyWorkingSetUpdate` 四个 standalone helper（2026-06-19 4-slice pull, R2 wiring-guard 通过 thin-delegate 保留）。目标是让 `LLMCodingRuntime` 成为可 review 的 orchestrator：主循环边界清楚，复杂决策由有 focused tests 的 helper / strategy 承担。`≤ 600 行` 只作为健康信号，不再是必须达成的硬目标；当剩余代码主要是在保持 event / compact / permission 顺序时，不再为了降行数继续拆分。下一步只在存在明确耦合风险、regression 风险或测试盲区时再切 `runExecuteStreamInner` / `tokenBudgetEnforcer` / `riskClassifier` 类边界。
...
7. **Stream G** — 存储层解耦（2026-06-20 起，**2026-06-21 收口**，已抽 8/8）。将 1753 行的大单体数据库管理类 `SqliteStorage.ts` 按领域拆分为主类做委托聚合：3B-20 `EventRepository`（206 行，events + listEvents/appendEvent + sequence allocation + duplicate-repair + tool-trace & execution-metrics callbacks）、3B-21 `TaskRepository`（166 行，tasks + saveTask/getTask/listTasks + JSON-column 序列化）、3B-22 `AuditRepository`（131 行，permission_audits + `approved`/`denied` decision enum）、3B-23 `ToolTraceRepository`（210 行，tool_traces + composite `started_at | tool_use_id` cursor pagination）、3B-24 `SessionChannelRepository`（316 行，session_channels + session_messages + cross-table inbox filter + read-modify-write acknowledge）、3B-25 `AgentJobRepository`（145 行，agent_jobs + dual storage `job_json` + indexed filter columns）、3B-26 `ExecutionMetricsRepository`（243 行，execution_metrics + 36 columns + boolean 0/1 encoding）、3B-27 `LoopPaneRepository`（181 行，loop_state + read-modify-write `updateLoopPaneRev`）。`SqliteStorage.ts` 已降到 **968 行（-785 / -44.8%）**，全部表操作都通过 thin delegation 转发到 8 个 repository；主类仅保留 schema setup、transaction lock、public method dispatch 和跨切面 helper（session row map、listAllEvents for SessionGetOptions）。Stream G 在该表面上不再有可被独立边界问题驱动的进一步抽取。
...
按 `Execution Slicing Addendum` 的 PR-sized map 继续推进。已收口:Phase 0.5 / 1A / 1B / 2A / 2B / 2C / 3A / **4A+(完整收口 — 含 2026-06-19 A1/A2/A3/C1/C2/C3/E1 七子切片,app.ts 864 → 226 → 191 行,累计 -77.9%,首次低于 ≤ 400 行目标)**。在执行中的:Phase 3B+(strategy class + 5 个 standalone helper 已抽出 — `eventsTranslator` / `behaviorTraceTap` / `loadWorkingSetOverride` / `applyWorkingSetUpdate` / `executeProviderRecoveryDecision`,LLMCodingRuntime 1841 → 1493 行 / -18.9%;主循环边界仍需审计)、Phase 2D (RuntimeServices 容器)、**Phase 9 Stream G 存储层解耦 (2026-06-21 收口,8 个 repository 全抽出,SqliteStorage 1753 → 968 行 / -44.8%,67 storage tests + 28 R-gate tests 覆盖契约,SQL UPSERT/WHERE/ORDER BY/JSON 序列化 byte-identical)**。Watch:Phase 5 (events / RuntimeEnv)、Phase 8 (tools → nexus 残留、SessionChannel 拆分)。下一步不是机械地把 `LLMCodingRuntime` 压到 600 行，而是审计主循环是否已经只剩 orchestration；若复杂决策仍内联，再按小 PR 抽出有测试边界的 helper / strategy；若剩余代码主要维护 event / compact / permission 顺序，则停止继续拆分。Stream G 在该表面上已收口：8 个 repository 已是聚焦的 reviewable + testable 单元，SqliteStorage 自身只剩 schema 迁移 + 9 个 delegation method + 跨切面 helper，不再有边界问题驱动的进一步抽取空间。`app.ts` 已降到 191 行,`parseSocketQuery` / `isLocalHost` / `validateSecurityConfig` tail cleanup 已完成。每一阶段都必须独立 PR、独立 regression、独立 review,且不得改动对外 REST / WebSocket 契约和 SQLite schema。不得通过"提高 `maxLoops`"、"放宽 `permissionMode`"、"忽略 `process.env` warning"等小动作来掩盖本计划要解决的耦合问题。
