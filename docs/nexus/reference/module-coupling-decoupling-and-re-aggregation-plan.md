...
| Phase 3 | Active Plan | Stream C `LLMCodingRuntime` decomposition. Extract `ProviderTurnDriver`, `ContextRefreshStrategy`, `ToolDispatchPipeline`, `HookDispatcher`, and standalone decision helpers where they remove review risk. The class body should read as an orchestrator; the 4 callers continue to work without change. | A new focused test per extracted class/helper exercises the boundary; the existing `test/runtime-llm.test.ts` and `test/runtime.test.ts` pass without modification; line count is tracked by `wc -l src/runtime/LLMCodingRuntime.ts` as a health signal. Phase 3 exits when the main loop boundary is clear and complex decisions are isolated with tests. The ≤ 600-line target is aspirational, not required if further extraction would mostly hide sequencing. |
