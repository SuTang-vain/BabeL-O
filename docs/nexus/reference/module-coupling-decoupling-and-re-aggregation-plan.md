...
| Phase 6 Watch — Events / RuntimeEnv | Watch (re-evaluated 2026-06-21) | `shared/events.ts` exhaustive translator (Stream F, Phase 6 north-star) and full `parseRuntimeEnv` rollout (Stream F, Phase 7 north-star). Original trigger "Defer until Phase 1-4 reduce hot spots" is now satisfied (Phase 1-4 all closed by 2026-06-19, Phase 9 closed 2026-06-21). Re-evaluation: Phase 7 (RuntimeEnv) is **unblocked and ready to land** as part of the 2D+7 single PR — see Phase 2 row above; Phase 6 (events codegen) remains Watch because the 333-line hand-written if/else in `src/runtime/eventsTranslator.ts` is reviewable today and the `Record<type, translator>` exhaustive-lookup migration is high-cost (~500-800 lines) for incremental value. | Phase 6 un-blocks when a new event type is added without translator coverage or a translator bug slips through review; Phase 7 lands with 2D as the single PR. |
...
| Phase 2 | In Progress 2026-06-21 (Open: 2D only) | Stream B singletons to `RuntimeServices`. 2A (2026-06-18 Closed) moved provider session rules to an injectable service; 2B (2026-06-18 Closed) removed the runtime hot-path dependency on `defaultContextBroadcaster`; 2C (2026-06-18 Closed) added explicit `ConfigManager` instances and migrated the first runtime / agent composition callsites. **2D Open**: introduce `src/runtime/services.ts` as the composition-root container (configManager / contextBroadcaster / everCoreManager / providerSessionRules / env) so the remaining module-level singletons (`defaultEverCoreRuntimeManager`, legacy `ConfigManager.getInstance()` callsites, `defaultContextBroadcaster` 7 refs) collapse into one typed container constructed once in `createDefaultNexusRuntime` and passed through `RuntimeExecuteOptions.services`. | Focused service-injection tests prove no cross-talk for each migrated service; `npm test` passes when the full Stream B migration closes. Once 2D lands, Phase 2 closes; no Stream B slice remains. |
