...
This plan is now governed by two canonical entry points:
...
- `SkillList`, `SkillShow`, `SkillValidate`, `SkillDraft`, and `SkillSave` remain narrow explicit capability names.
- Skill tools should return structured failures and diagnostics instead of throwing opaque runtime errors where recoverable.
- Skill save paths must respect test isolation and environment-driven config/storage roots.
- `skill_matched`, `skill_invoked`, `skill_validation`, and `skill_saved` events should follow the shared session event envelope.
- Generated skills must be drafts first, then validated, previewed, and explicitly saved by scope.

## 中文概述

### 背景

BabeL-O 已有轻量 skill substrate：Markdown skill 文件、触发词匹配、系统提示注入和 compact retention。但完整产品闭环还缺少显式 list/show/validate/invoke/draft/save。

### 边界

Skill 不是隐藏 policy，也不能绕过工具权限、workspace 边界或 evidence 要求。自动生成 skill 必须先生成草案，再验证、预览、确认保存；不得静默覆盖已有 skill。
