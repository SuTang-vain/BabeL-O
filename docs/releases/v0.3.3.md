...
3. **TUI visual and interaction upgrades**
   ...
   - First-time model setup is clearer: `/model` now guides users through provider credentials before model selection, and the welcome panel surfaces `auth setup /model` when a remote provider still needs an API key.

4. **Provider configuration guardrails**
   - `/model` and `bbl config use <modelId>` now block API-key providers until credentials are configured, while no-auth providers such as `local/coding-runtime` and `ollama/*` remain selectable.
   - Provider authentication failures now include a BabeL-O-side hint such as `bbl config add <provider> <KEY>` instead of only showing raw provider 401 text.

5. **Context and tool panels**
...
3. **TUI 视觉与交互升级**
   ...
   - 首次模型配置更清晰：`/model` 会先引导配置 provider credentials，再进入模型选择；welcome 面板会在远程 provider 缺少 API key 时显示 `auth setup /model`。

4. **Provider 配置护栏**
   - `/model` 与 `bbl config use <modelId>` 现在会在 API-key provider 未配置 credentials 时阻止保存；`local/coding-runtime` 与 `ollama/*` 这类不需要 key 的 provider 仍可直接选择。
   - provider 鉴权失败时会显示 BabeL-O 侧提示，例如 `bbl config add <provider> <KEY>`，不再只显示原始 provider 401 文本。
