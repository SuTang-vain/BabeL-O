...
4. **Release workflow hardening**
   ...
   - GitHub Actions now builds portable packages for macOS arm64, macOS x64, Linux x64, and Windows x64.
...
Windows users should download `bbl-windows-x64.tar.gz` from the release, extract it, and run:
...
- [Windows x64: bbl-windows-x64.tar.gz](https://github.com/SuTang-vain/BabeL-O/releases/download/v0.3.5/bbl-windows-x64.tar.gz)
...
- `curl | bash` supports macOS and Linux. Windows is available through manual package extraction.
...
4. **Release 工作流加固**
   ...
   - CI 会构建 macOS arm64、macOS x64、Linux x64、Windows x64 的 portable 包。
...
Windows 用户请下载 `bbl-windows-x64.tar.gz`，解压后运行：
