...
export async function runEverOSMemorySetup(options: EverOSSetupOptions = {}): Promise<EverOSSetupResult> {
  ...
  if (existing.ok && existing.state?.buildStatus === 'ready' && !options.retry) {
    stdout.write(`${chalk.green('✓')} MemoryOS bootstrap is already ready at ${existing.path}\n`)
  ...
  stdout.write(`${chalk.green('✓')} MemoryOS local memory bootstrap is ready.\n`)
  stdout.write(chalk.dim(`  command: ${managedCommand}\n  dataDir: ${dataDir}\n`))
...
export async function formatEverOSMemorySetupStatus(env: NodeJS.ProcessEnv = process.env): Promise<string> {
  ...
  if (!read.ok) {
    return [
      chalk.red('MemoryOS bootstrap state is invalid.'),
      `path: ${read.path}`,
  ...
  if (!read.exists || !read.state) {
    return [
      chalk.yellow('MemoryOS bootstrap has not been configured.'),
      `path: ${read.path}`,
