...
test('allocateBudget uses adaptive large-context ceiling and honors env cap', async () => {
  ...
  try {
    ...
    assert.equal(cappedAnalysis.diagnostic.details.modelContextWindow, 1_000_000)
...
test('/context display includes cache-aware long-context diagnostics', async () => {
  ...
  try {
    ...
    assert.equal(analysis.diagnostics.cacheEconomics.effectiveContextCeiling, 920_000)
    assert.equal(analysis.diagnostics.cacheEconomics.reservedOutputTokens, 16_384)
    assert.equal(analysis.diagnostics.cacheEconomics.providerSafetyBufferTokens, 20_000)
    ...
    assert.equal(analysis.diagnostics.cacheEconomics.compactThresholdTokens, Math.floor(920_000 * 0.93))
    assert.equal(analysis.diagnostics.cacheEconomics.blockingLimitTokens, Math.max(Math.floor(920_000 * 0.93), 920_000 - 1_000))
...
test('assembleContext injects MemoryProvider results as volatile long-term memory', async () => {
  ...
  assert.match(context.systemPrompt, /Capability: long_term_memory/)
  assert.match(context.systemPrompt, /State: available for this session/)
  assert.match(context.systemPrompt, /Source: runtime memory provider diagnostics/)
  assert.match(context.systemPrompt, /Tool surface: memory_search, memory_save_note, memory_flush_session/)
  assert.match(context.systemPrompt, /Allowed triggers:/)
  assert.match(context.systemPrompt, /Use memory_search when the user asks about prior preferences/)
  assert.match(context.systemPrompt, /Risk boundary:/)
  assert.match(context.systemPrompt, /Authority:/)
  assert.match(context.systemPrompt, /User-facing policy:/)
